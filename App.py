...
st.set_page_config(page_title="Central de Mistura Wesley", page_icon="🚜", layout="wide")
...
with st.sidebar:
    ...
    for i in range(n_prod):
        ...
        p_ref = st.selectbox(f"Produto", list(DB_PRODUTOS.keys()), index=list(DB_PRODUTOS.keys()).index(p_ref_val := p_def if p_def in DB_PRODUTOS else "Outro (Novo)"), key=f"sel_{i}")
...
if sobra > 0:
    ...
    st.link_button("📲 Enviar via WhatsApp (Final)", preparar_zap(sobra, f"ÚLTIMA BATIDA ({int(sobra)}L)"))
